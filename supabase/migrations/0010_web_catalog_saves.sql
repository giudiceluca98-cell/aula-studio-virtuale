-- Private catalog saves from Eve web-search citations.

begin;

drop policy if exists catalog_materials_read_active on public.catalog_materials;
create policy catalog_materials_read_active on public.catalog_materials for select to authenticated
using (
  is_active
  and verification_status <> 'rejected'
  and (
    verification_status in ('verified', 'official_source')
    or created_by = (select auth.uid())
    or exists (
      select 1 from public.saved_catalog_materials s
      where s.material_id = id and s.user_id = (select auth.uid())
    )
  )
);

drop policy if exists catalog_material_topics_read on public.catalog_material_topics;
...
using (
  exists (
    ...
    where m.id = material_id
      ...
      and (
        m.verification_status in ('verified', 'official_source')
        or m.created_by = (select auth.uid())
        or exists (
          select 1 from public.saved_catalog_materials s
          where s.material_id = m.id and s.user_id = (select auth.uid())
        )
      )
  )
);
...
begin
  ...
  if char_length(pg_catalog.btrim(p_title)) not between 1 and 240
     ...
     or p_source_url ~* '^https://(?:localhost|127\.|10\.|0\.|169\.254\.|192\.168\.|172\.(?:1[6-9]|2[0-9]|3[01])\.)' then
  ...
  if v_material is null then
    begin
      insert into public.catalog_materials(
        ...
        pg_catalog.btrim(p_provider), p_source_url, 'article',
        pg_catalog.lower(pg_catalog.btrim(p_language)), 'beginner', 'unknown',
        'pending', 'community', 'external', v_user, true
...
revoke all on function public.save_web_result_to_catalog(text,text,text,text,text) from public, anon;
grant execute on function public.save_web_result_to_catalog(text,text,text,text,text) to authenticated;
