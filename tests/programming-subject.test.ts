...
describe("pacchetto editoriale Programmazione da zero", () => {
  ...
  it("pubblica le lezioni ufficiali disponibili nei Moduli 0 e 1", () => {
    ...
    expect(programmingSubjectPackage.stages[1].lessons).toEqual(programmingCurriculumOutline.slice(9));
    expect(programmingLesson.modules).toHaveLength(2);
    ...
    expect(programmingLesson.modules[1].lessons.map((lesson) => lesson.id)).toEqual(["1.1", "1.2", "1.3", "1.4", "1.5", "1.6", "1.7"]);
    for (const lesson of programmingLesson.modules[1].lessons) {
      expect(lesson.sectionIds).toHaveLength(11);
      expect(lesson.exerciseIds).toHaveLength(["1.6", "1.7"].includes(lesson.id) ? 60 : 40);
      expect(lesson.quizIds).toHaveLength(30);
      expect(lesson.glossary).toHaveLength(["1.6", "1.7"].includes(lesson.id) ? 60 : 50);
      expect(lesson.objectives).toHaveLength(["1.3", "1.4", "1.5", "1.6", "1.7"].includes(lesson.id) ? 10 : 8);
    }
  });

  it("estende i contenuti ufficiali fino alla lezione 1.7", () => {
    ...
    expect(programmingLesson.objectives).toHaveLength(140);
    expect(programmingLesson.sections).toHaveLength(182);
    expect(programmingLesson.glossary).toHaveLength(850);
    expect(programmingLesson.exercises).toHaveLength(703);
    expect(programmingLesson.quiz).toHaveLength(498);
    expect(programmingLesson.project.assessments).toHaveLength(16);
    ...
    expect(programmingLesson.completion.requiredExerciseIds).toHaveLength(497);
    expect(new Set(programmingLesson.sections.map((section) => section.id)).size).toBe(182);
    expect(new Set([programmingLesson.guidedExercise.id, ...programmingLesson.exercises.map((exercise) => exercise.id)]).size).toBe(704);
    expect(new Set(programmingLesson.quiz.map((question) => question.id)).size).toBe(498);
  ...
  it("crea un percorso con lezione nativa, checklist e approfondimenti facoltativi", () => {
    ...
    expect(draft.modules[1].items.filter((item) => item.itemType === "exercise")).toHaveLength(7);
    expect(draft.modules[1].items.filter((item) => item.itemType === "checkpoint")).toHaveLength(7);
