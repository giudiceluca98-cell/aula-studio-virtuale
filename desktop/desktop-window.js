(() => {
  ...
  async function setFullscreen(nextValue) {
    ...
    try {
      ...
      document.body.dataset.desktopFullscreen = String(nextValue);
    } catch (error) {
  ...
  document.addEventListener("keydown", (event) => {
    ...
    if (event.key === "Escape" && document.body.dataset.desktopFullscreen === "true") {
      event.preventDefault();
  ...
  currentWindow
    .isFullscreen()
    .then((value) => {
      document.body.dataset.desktopFullscreen = String(value);
    })
    .catch(() => {});

  window.AulaDesktopWindow = {
