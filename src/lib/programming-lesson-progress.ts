...
export function eveLessonAdvice(stateValue: unknown) {
  ...
  if (state.quizCompleted && (state.quizScore ?? 0) < programmingLesson.completion.minimumQuizScore) return { title: "Ripasso mirato", message: `Il quiz ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¨ al ${state.quizScore ?? 0}%. Rivedi i capitoli associati alle risposte errate e riprova.`, sectionIds: [...new Set(incorrect.map((answer) => answer.reviewSectionId))] };
  const nextSection = programmingLesson.sections.find((section) => !state.completedSectionIds.includes(section.id));
  if (nextSection) return { title: "Prossimo passo", message: `Continua da ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã¢â‚¬Å“${nextSection.title}ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â e segnala la sezione come compresa quando sapresti rispiegarla.`, sectionIds: [nextSection.id] };
  if (state.guidedExercise !== "completed") return { title: "Metti in pratica", message: `Completa lÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â‚¬Å¾Ã‚Â¢esercizio guidato del capitolo ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã¢â‚¬Å“${programmingLesson.guidedExercise.title}ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â.`, sectionIds: ["programming-0-1-chapter-1"] };
  const nextExercise = programmingLesson.exercises.find((exercise) => !state.independentExerciseIds.includes(exercise.id));
  if (nextExercise) return { title: "Esercizio consigliato", message: `Prosegui con ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã¢â‚¬Å“${nextExercise.title}ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â.`, sectionIds: [] as string[] };
  if (!state.quizCompleted) return { title: "Verifica i concetti", message: "Avvia o completa i quiz. Eve userÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â  gli errori per indicarti i capitoli da ripassare.", sectionIds: [] as string[] };
