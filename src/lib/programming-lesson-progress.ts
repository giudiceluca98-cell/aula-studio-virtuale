...
export function eveLessonAdvice(stateValue: unknown) {
  ...
  if (state.quizCompleted && (state.quizScore ?? 0) < programmingLesson.completion.minimumQuizScore) return { title: "Ripasso mirato", message: `Il quiz Ã¨ al ${state.quizScore ?? 0}%. Rivedi i capitoli associati alle risposte errate e riprova.`, sectionIds: [...new Set(incorrect.map((answer) => answer.reviewSectionId))] };
  const nextSection = programmingLesson.sections.find((section) => !state.completedSectionIds.includes(section.id));
  if (nextSection) return { title: "Prossimo passo", message: `Continua da â€œ${nextSection.title}â€ e segnala la sezione come compresa quando sapresti rispiegarla.`, sectionIds: [nextSection.id] };
  if (state.guidedExercise !== "completed") return { title: "Metti in pratica", message: `Completa lâ€™esercizio guidato del capitolo â€œ${programmingLesson.guidedExercise.title}â€.`, sectionIds: ["programming-0-1-chapter-1"] };
  const nextExercise = programmingLesson.exercises.find((exercise) => !state.independentExerciseIds.includes(exercise.id));
  if (nextExercise) return { title: "Esercizio consigliato", message: `Prosegui con â€œ${nextExercise.title}â€.`, sectionIds: [] as string[] };
  if (!state.quizCompleted) return { title: "Verifica i concetti", message: "Avvia o completa i quiz. Eve userÃ  gli errori per indicarti i capitoli da ripassare.", sectionIds: [] as string[] };
