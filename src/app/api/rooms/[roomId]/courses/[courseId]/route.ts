...
import { courseRemovalRequestSchema, parseCleanupJobs, roomContentIdSchema } from "@/lib/room-content-removal";
...
async function context(params: Promise<{ roomId: string; courseId: string }>) {
  const values = await params;
  if (!roomContentIdSchema.safeParse(values.roomId).success || !roomContentIdSchema.safeParse(values.courseId).success) return null;
  return values;
}

export async function GET(_request: NextRequest, { params }: { params: Promise<{ roomId: string; courseId: string }> }) {
  const values = await context(params);
  if (!values) return json({ error: "invalid_resource" }, 400);
  const supabase = await createClient();
  const { data: { user } } = await supabase.auth.getUser();
  if (!user) return json({ error: "not_authenticated" }, 401);
  const { data, error } = await supabase.rpc("get_course_removal_impact", { p_course_id: values.courseId, p_room_id: values.roomId });
  if (error) return json({ error: error.code === "42501" ? "not_authorized" : "impact_failed" }, error.code === "42501" ? 403 : 409);
  return json({ impact: data });
}

export async function DELETE(request: NextRequest, { params }: { params: Promise<{ roomId: string; courseId: string }> }) {
  if (!isSameOriginRequest(request)) return json({ error: "origin_not_allowed" }, 403);
  const values = await context(params);
  if (!values) return json({ error: "invalid_resource" }, 400);
  if (Number(request.headers.get("content-length") ?? 0) > 1024) return json({ error: "payload_too_large" }, 413);
  let body: unknown;
  try { body = await request.json(); } catch { return json({ error: "invalid_json" }, 400); }
  const parsed = courseRemovalRequestSchema.safeParse(body);
  if (!parsed.success) return json({ error: "invalid_payload" }, 400);

  const supabase = await createClient();
  const { data: { user } } = await supabase.auth.getUser();
  if (!user) return json({ error: "not_authenticated" }, 401);
  const { data, error } = await supabase.rpc("remove_room_course", {
