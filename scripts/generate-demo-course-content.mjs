...
const payloadSha256 = sha256(output);
const adapterSha256 = sha256(adapter);

if (adapter.toString("utf8").toLowerCase().includes("</script")) {
  throw new Error("L'adattatore contiene una chiusura script non incorporabile");
}
...
await writeFile(integratedDemoPath, integratedDemo, "utf8");
